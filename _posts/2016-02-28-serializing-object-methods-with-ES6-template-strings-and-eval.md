...
difference between writing an elegant, easy to reason about application, 
and a crude hack. As far as my experience goes, 9 out of 10 teams who fail 
to find a reasonable solution to a problem in 1 or 2 meetings, abandon the 
investigation and resort to third-party solutions &mdash; this is not always 
the best case.

Now, don't get me wrong, I am all in favor of libraries and try as hard as I can 
not to reinvent the wheel every time I work on a new project, but at the same 
time, I strongly believe that once you have a set of tools that you've built, you can 
reuse them on every future project that needs it.
...
&mdash; values &mdash; get serialized, and the methods get left behind.
...
and the cost I had to pay was believing too much in "Eval is evil".
...
accross "the wire" and can be easily understood and generated by both 
...
`person` object, includinc its method.
...
transportation "accross the wire". What we need to do next is to de-serialize 
...
// => text(required) - the string we whish to de-serialize and convert back
...
way we used to mimick this in ES5.
